...
class TEM(tf.keras.Model):
    ...
    def step(self, inputs, g_t, x_t, variable_dict, i, t_mat, g_t_spike, ca3_prev, p2g_prev, ca1_prev, mem_offset=0):
        ...
        g_gen, g2g_spike = self.gen_g(g_t, t_mat, seq_pos, inputs.d[i], g_t_spike, inputs.positions[i])
        ...
        var_updates = [[['p', 'p'], p],
                       ...
                       [['p', 'dg'], tf.reduce_mean(dg_all, axis=2)],
    ...
    def infer_g(self, g_gen, mu_x2p, x, g2g_spike, x2g_spike, p2g_prev, g_t_spike):
        ...
        """mu = g_gen + tf.reduce_mean(p2g_prev, axis=2) *self.p2g + tf.reduce_mean(x2g_spike, axis=2) *self.p_p#
    ...
    @model_utils.define_scope
    def g2g(self, g, t_mat, d, g_spike, pos, name=''):
        ...
        #mu = tf.clip_by_value(mu, clip_value_min=0, clip_value_max=1)
        #####################################
        g = mu[..., tf.newaxis] #* dt  # shape = [B, D, 1]
        g = tf.tile(g, [1, 1, self.spike_step])  
        g2g_spike = self.poisson_spike(g*self.g2_spike)"""
        mu_all2 = tf.TensorArray(dtype=tf.float32, size=self.spike_step)
        #mu_all = tf.TensorArray(dtype=tf.float32, size=self.spike_step)
        v2 = self.v_g2g
        step = 0
        for i in range(self.spike_step):
            ...
            o, v2 = self.get_g2g_update2(g_spike[:,:,i]*self.g2g_spike, t_mat, v2, pos, step)
            #step += 1
    ...
    @model_utils.define_scope
    def get_g2g_update2(self, g_p, t_mat, v_prev, pos, step):
        ...
        v_new = alpha * v_prev + input_current #- self.par['theta_amp'] * (tf.sin(np.pi/2+2.0 * np.pi * step  /self.spike_step)+1)
...
def compute_losses(model_inputs, data, trainable_variables, par):
    ...
    for i in range(par.seq_len):
        ...
        lp_reg_ = 2*tf.reduce_sum(tf.abs(data.p.p_g[i]), axis=1) + 1*tf.reduce_sum(tf.abs(data.p.p[i]), axis=1) + 2*tf.reduce_sum(tf.abs(data.p.dg[i]), axis=1) + 0*tf.reduce_sum(tf.abs(data.p.ca3[i]), axis=1)
