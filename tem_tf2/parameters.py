...
def default_params(width=None, height=None, world_type=None, batch_size=None):
    ...
    params.save_interval = int(50000 / params.seq_len) #20000/
    ...
    params.n_grids_all = [90] #[30, 30, 24, 18, 18]
    ...
    params.freqs = sorted([0.01, 0.7, 0.91, 0.97, 0.99, 0.9995])[:params.n_freq] #[0.01, 0.7, 0.91, 0.97, 0.99, 0.9995]
...
def get_env_params(par, width, height):
    if par.world_type == 'rectangle':
        par_env = Dd({'stay_still': True,
                      ...
                      'widths': env_width if not width else
                      [width] * par.batch_size,
                      'heights': env_width if not height else
                      [height] * par.batch_size,
...
def old2new(world_type):
    old2new_name_convert = Dd({'rectangle': 'rectangle',
                               'square': 'rectangle',
                               })
    try:
