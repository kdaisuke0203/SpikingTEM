...
def train_step(model_, model_inputs_):
    ...
    print('train_step_optimiser_done2')
...
for train_i in range(params.train_iters): #for train_i in range(params.train_iters):
    ...
    if debug_data:
        ...
        #print("#######")
    stop_time = time.time()
    #print("model_start_time,data_start_time,stop_time",model_start_time,data_start_time,stop_time)
    # Update logging info
    #print("Update logging info")
    ...
    """if train_i % params.sum_int_inferences == 0 and train_i > 0:
        print("Log current model performance summaries")
    ...
    if train_i == params.train_iters-1:
        np.save(save_path + '/params', dict(params))
