...
class TEM(tf.keras.Model):
    def __init__(self, par):
        ...
        self.w_g = tf.Variable(1.0, dtype=self.precision, trainable=True, name='w_g')
        self.w2_g = tf.Variable(1.0, dtype=self.precision, trainable=True, name='w2_g')
    ...
    def infer_g(self, g2g_all, mu_x2p, x, memories):
        ...
        if 'p' in self.par.infer_g_type:
            mu_p2g, p_x = self.p2g(mu_x2p, x, memories)
            if len(tf.shape(sigma))==3:
                sigma=tf.reduce_mean(sigma,axis=2)
            mu = mu * self.w_g + mu_p2g * self.w2_g #* sigma_p2g
    ...
    def p2g(self, x2p, x, memories):
        ...
        ############ spiking ###########################
        mus = [self.p2g_mu_spiking[f](tf.transpose(tf.tile(tf.expand_dims(x, axis=2), multiples=[1,1,self.spike_step]), perm=[0, 2, 1])) for f, x in enumerate(mu_attractor_sensum_)]
        mus = [tf.transpose(mus[f], perm=[0, 2, 1]) for f, x in enumerate(mu_attractor_sensum_)]
        if len(tf.shape(mus[0]))==3:
