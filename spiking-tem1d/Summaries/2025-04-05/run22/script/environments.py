...
class Rectangle(Environment):
    ...
    def walk(self):
        ...
        for i in range(time_steps - 1):
            ...
            #print("self.tran[int(position[i])",self.tran[int(position[i]), :])
            #print("i",i)
            ...
            if left2right == False:
                action_dir = np.random.choice(5, size=1, p=[0.0, 0.0, 0, 1, 0]) # stop, south, north, west, east
            else:
                action_dir = np.random.choice(5, size=1, p=[0.0, 0.0, 0.0, 0.0, 1])
            ...
            """if rep_num_k < rep_num * self.width:
                ...
                if rep_num_k % rep_num == 0 and position[i] - position[i-1] == 1 and rep_num_k < rep_num * (self.width + self.height):
                    move_dir = 'south"""
            #print("rep_num_k",rep_num_k)
            #print("left2right",left2right)
