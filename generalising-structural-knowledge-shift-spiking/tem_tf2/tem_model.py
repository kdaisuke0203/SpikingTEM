...
class TEM(tf.keras.Model):
    ...
    def p2g(self, x2p, x, memories):
        ...
        #print("X2P",x2p.shape) #shape(env_num, p_num)
        #print("memo",memories.shape)
        p_x = self.attractor(x2p, memories) #x2p=[env_num, p_num], p_x=[env_num, p_num]
        #print("P_X",p_x.shape) #shape(env_num, p_num)

        # sum over senses
        #print("self.batch_size, self.par.tot_phases, self.par.s_size_comp",self.batch_size, self.par.tot_phases, self.par.s_size_comp)
        mu_attractor_sensum = tf.reduce_mean(
            tf.reshape(p_x, (self.batch_size, self.par.tot_phases, self.par.s_size_comp)), axis=2)

        #print("mu_attractor_sensum",mu_attractor_sensum.shape)
    ...
    def x2x_(self, x, x_, d):
        ...
        for f in range(self.par.n_freq):
            ...
            x_s.append(filtered)
            #print("x_s0",f,x_s)
        #print("x_s",x_s) #shape(env_num, params.s_size_comp)
        return x_s
    ...
    def gen_g(self, g, t_mat, seq_pos):
        ...
        #print("mu_gen",mu_gen) #shape(env_num, g_num)

        mu_prior, sigma_prior = self.g_prior()
        #print("mu_prior",mu_prior) #shape(env_num, g_num)
    ...
    def g2g(self, g, t_mat, name=''):
        ...
        #print("mu",mu) ##shape(env_num, g_num)
    ...
    def f_x(self, p):
        ...
        #print("x",x) ##shape(env_num, params.s_size)
