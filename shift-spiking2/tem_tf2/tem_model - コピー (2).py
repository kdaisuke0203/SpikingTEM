...
class TEM(tf.keras.Model):
    ...
    def get_transition(self, d):
        ...
        if len(tf.shape(d))==3:
            trans_all = tf.reshape(t_vec, [self.batch_size, self.par.g_size, self.par.g_size, self.spike_step])
            mask_g2 = tf.tile(tf.expand_dims(self.mask_g, axis=2), multiples=[1,1,self.spike_step])
            # apply mask - i.e. if hierarchically or only transition within frequency
            return trans_all * mask_g2
    ...
    def get_g2g_update(self, g_p, t_mat):
        ...
        #print("g_p1",g_p)
        #print("g_p2",tf.expand_dims(g_p, axis=2))
        #print("tmat",t_mat)
        ...
        if len(tf.shape(g_p))==3:
            #values = tf.TensorArray(dtype=self.precision, size=0, dynamic_size=True) 
            values = tf.zeros([tf.shape(g_p)[0],tf.shape(g_p)[1], tf.shape(g_p)[2]])
            #print("g_p[i,:,0]",g_p[0,:,:])
